...
def get_mean_std():
    train_sig_data = torch.load('../Stocks/data/rolling_train_data.pt')
    column_mean = torch.mean(train_sig_data, dim = 0)
    column_std = torch.std(train_sig_data, dim = 0)
    return column_mean, column_std

# Data prep
data = torch.load(os.path.join('..','Stocks', 'data', 'rolling_train_data.pt')).float()                 # Loads data from Stocks folder
